...
while section_height <= maximum_height:
    ...
    for product, shape in product_shapes:
        ...
        if len(section_edges) > 0:
            ...
            edges_handle = OCC.Core.TopTools.Handle_TopTools_HSequenceOfShape_DownCast(edges)

            wires = OCC.Core.TopTools.TopTools_HSequenceOfShape()
            wires_handle = OCC.Core.TopTools.Handle_TopTools_HSequenceOfShape_DownCast(wires)
            ...
            for i in range(wires.Length()):
                ...
                wire_data_handle = OCC.Core.ShapeExtend.Handle_ShapeExtend_WireData_DownCast(wire_data)
