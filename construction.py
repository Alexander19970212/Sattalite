...
class Balance_mass:
    ...
    def initial(self, frame, modules, faces, limits):
    ...
    def random_location2(self):
        print('Start randomMMMMMMMMMMMMMMMMMMMMMMMMM')
        import random
        for name in self.reserv_models:
            res = 0.99
            while res >= 0.99:

                number = np.random.randint(0, len(self.limits[name][0]))
                rotation = np.random.randint(0, len(self.limits[name][1]))
                number = self.limits[name][0][int(number)]
                rotation = math.radians(self.limits[name][1][int(rotation)])

                d_x = np.random.sample() * 2 - 1
                d_y = np.random.sample() * 2 - 1
                print('step: ', name, ' ', number, ' ', rotation, ' ', d_x, ' ', d_y)
                #№%%%%%%%%%%%%%%%%%%%%goal_functino2
                self.Locate_centre_face(number, name, rotation, d_x, d_y)
